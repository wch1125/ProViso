        ...
        <div className="flex items-center gap-2">
          ...
          {hasActiveFilters && (
            <button
              ...
              className="flex items-center gap-1 px-3 py-2 text-sm text-text-tertiary hover:text-text-primary transition-colors"
            >
              <X className="w-3 h-3" />
              Clear
            </button>
      ...
      {options.map((option) => (
        ...
        >
          ...
          {option.count !== undefined && (
            <span className={`ml-1.5 ${value === option.id ? 'text-gold-200' : 'text-text-muted'}`}>
              ({option.count})
            </span>
          )}
        </button>
